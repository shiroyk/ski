...
func setup() {
	...
	mux.HandleFunc("/lastmodified", func(w http.ResponseWriter, r *http.Request) {
		lm := "Fri, 14 Dec 2010 01:01:50 GMT"
		if r.Header.Get("if-modified-since") == lm {
			w.WriteHeader(http.StatusNotModified)
			return
	...
	mux.HandleFunc("/varyaccept", func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Cache-Control", "max-age=3600")
	...
	mux.HandleFunc("/doublevary", func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Cache-Control", "max-age=3600")
		w.Header().Set("Content-Type", "text/plain")
		w.Header().Set("Vary", "Accept, Accept-Language")
		_, _ = w.Write([]byte("Some text content"))
	})
	mux.HandleFunc("/2varyheaders", func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Cache-Control", "max-age=3600")
		w.Header().Set("Content-Type", "text/plain")
	...
	mux.HandleFunc("/varyunused", func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Cache-Control", "max-age=3600")
		w.Header().Set("Content-Type", "text/plain")
	...
	mux.HandleFunc("/cachederror", func(w http.ResponseWriter, r *http.Request) {
		etag := "abc"
		if r.Header.Get("if-none-match") == etag {
			w.WriteHeader(http.StatusNotModified)
			return
	...
	mux.HandleFunc("/updatefields", func(w http.ResponseWriter, r *http.Request) {
	...
	mux.HandleFunc("/3seconds", func(w http.ResponseWriter, r *http.Request) {
	...
	mux.HandleFunc("/infinite", func(w http.ResponseWriter, r *http.Request) {
		for {
			...
			case <-s.done:
				return
			default:
				if _, err := w.Write([]byte("Some text content")); err != nil {
					return
				}
				_, _ = w.Write([]byte{0})
			}
...
// but modern http.Client uses RequestConfig.Cancel (or request context) instead,
