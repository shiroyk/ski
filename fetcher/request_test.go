...
{{ get "image" }}`, "image/png"},
	{`POST {{.url}} HTTP/1.0
Content-Type: multipart/form-data; boundary=X-1141541919810
...
{{ get "image" }}
--X-1141541919810--`, "foo-test.png-image/png"},
}
...
func TestNewTemplateRequest(t *testing.T) {
	h := http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case "multipart/form-data; boundary=X-1141541919810":
